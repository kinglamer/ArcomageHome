...
namespace Arcomage.Tests.Moq
{
    internal class TestServerForCustomCard : IArcoServer
    {
        public string GetRandomCard()
        {
            ...
            var paramsM56 = new List<CardParams>();
            paramsM56.Add(new CardParams() {key = Specifications.PlayAgain, value = -50});
            paramsM56.Add(new CardParams() {key = Specifications.CostAnimals, value = 0});

            returnVal.Add(new Card()
            {
                id = 56,
                name = "Check new card params3",
                cardParams = paramsM56
            });
            ...
            returnVal.Add(new Card()
            {
                id = 7,
                name = "null",
                cardParams = new List<CardParams> {new CardParams() {key = Specifications.CostAnimals, value = 100}}
            });
